# Ops runbook (Flare `fce-sign` scaffold)

Deploy, registration, and troubleshooting tooling inherited from Flare's
`fce-sign` scaffold — a private-key-manager example unrelated to this
submission's RFQ matcher (see [`README.md`](README.md)). Kept because this
build has actually used it, more than once, to get the Go extension onto
Coston2.
...
> which stores no secrets on-chain.)
...
This submission's RFQ matcher uses none of these directly — the extension's
`/direct` path hard-rejects any `OPType` other than `RFQ` (see
[`../docs/TRUST.md`](../docs/TRUST.md)); `KEY` management only works via this
on-chain contract, where it costs real gas and leaves an auditable sender.
...
./scripts/test.sh              # 4. Scaffold's own smoke test — signs with UPDATE/SIGN
                                #    against the running TEE. Exercises the shared
                                #    key-manager path, not the RFQ matcher; a passing
                                #    run confirms the TEE and signer key are alive.
...
| Doc                                                  | What it covers                                     |
| ----------------------------------------------------- | --------------------------------------------------- |
| [`README.md`](README.md)                              | What this directory actually ships (the RFQ matcher) |
