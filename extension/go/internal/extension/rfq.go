...
type rfqBook struct {
	...
	usedNonces map[common.Address]map[string]bool // taker -> nonce.String() -> used
}

func newRfqBook() *rfqBook {
	return &rfqBook{
		open:       make(map[common.Hash]*rfqState),
		usedNonces: make(map[common.Address]map[string]bool),
	}
}
...
// --- Handlers, dispatched from processAction in extension.go ---
...
func (b *rfqBook) openRfq(eip712 eip712Config, intent rfqIntent) (common.Hash, error) {
	...
	if b.usedNonces[intent.Taker][nonceKey] {
	...
	if b.usedNonces[intent.Taker] == nil {
		b.usedNonces[intent.Taker] = make(map[string]bool)
	}
	b.usedNonces[intent.Taker][nonceKey] = true
...
func submitSettle(rfqID common.Hash, intent rfqIntent, winningQuote quote, expiry uint64, attestationSig []byte) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("parsing RFQ_HOT_KEY: %w", err)
	}
	...
	auth, err := bind.NewKeyedTransactorWithChainID(hotKey, chainID)
	if err != nil {
	...
	tx, err := contract.Settle(auth, fill, attestationSig)
	if err != nil {
